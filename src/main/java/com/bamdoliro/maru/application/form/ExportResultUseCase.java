...
public class ExportResultUseCase {
    ...
    public Resource execute() throws IOException {
        ...
        List<FormColumn> columnList = List.of(
                ...
                FormColumn.text(form -> form.getGrade().getSubjectList().getSubjectMap().getSubjectListOf(2, 1).size()),
                FormColumn.text(form -> form.getGrade().getSubjectList().getSubjectMap().getSubjectListOf(2, 1).totalScore()),
                FormColumn.text(form -> form.getGrade().getSubjectList().getSubjectMap().getSubjectListOf(2, 2).size()),
                FormColumn.text(form -> form.getGrade().getSubjectList().getSubjectMap().getSubjectListOf(2, 2).totalScore()),
                FormColumn.text(form -> form.getGrade().getSubjectList().getSubjectMap().getSubjectListOf(3, 1).size()),
                FormColumn.text(form -> form.getGrade().getSubjectList().getSubjectMap().getSubjectListOf(3, 1).totalScore()),
        ...
        return xlsxGenerator.execute("전체결과", formList, columnList);
    }
}
