...
class UserControllerTest extends RestDocsTestSupport {
    ...
    @Test
    void 유저를_생성할_때_인증하지_않은_전화번호거나_만료된_전화번호라면_에러가_발생한다() throws Exception {
        doThrow(new VerifyingHasFailedException()).when(signUpUserUseCase).execute(any(SignUpUserRequest.class));
        SignUpUserRequest request = new SignUpUserRequest("01085852525", "김밤돌", "ABC123", "password123$");
    ...
    void 전화번호_인증을_요청한다() throws Exception {
        VerificationRequest request = new VerificationRequest("01085852525");
        willDoNothing().given(sendVerificationUseCase).execute(any(VerificationRequest.class));
        ...
        verify(sendVerificationUseCase, times(1)).execute(any(VerificationRequest.class));
    }

    @Test
    void 전화번호_인증을_요청할_때_잘못된_형식의_전화번호를_보내면_에러가_발생한다() throws Exception {
        VerificationRequest request = new VerificationRequest("누가봐도전화번호아님ㅎg");
        willDoNothing().given(sendVerificationUseCase).execute(any(VerificationRequest.class));
        ...
        verify(sendVerificationUseCase, never()).execute(any(VerificationRequest.class));
    }

    @Test
    void 전화번호_인증을_요청할_때_전화번호_전송이_실패하면_에러가_발생한다() throws Exception {
        VerificationRequest request = new VerificationRequest("010아무도안쓰는번호");
        doThrow(new FailedToSendException())
                .when(sendVerificationUseCase).execute(any(VerificationRequest.class));
